...
class TierValidation(models.AbstractModel):
    ...
    def _get_sequences_to_approve(self, user):
        """Get sequences that the user can approve, with strict permission checking."""
        all_reviews = self.review_ids.filtered(
            lambda r: r.status in ("waiting", "pending")
        )
        my_reviews = all_reviews.filtered(lambda r: user in r.reviewer_ids)
        
        # For multiple_individuals review type, also check specific_reviewer_id
        my_reviews = my_reviews.filtered(
            lambda r: not r.definition_id.review_type == "multiple_individuals" or
                     (r.specific_reviewer_id and r.specific_reviewer_id == user)
        )
    ...
    def validate_tier(self):
        ...
        # 强制限制为单条记录（示例）
        if len(reviews) != 1:
            raise ValidationError("当前存在多条待处理评审记录，请检查配置！")
        if self.has_comment:
            user_reviews = reviews.filtered(
                lambda r: r.status == "pending" and (self.env.user in r.reviewer_ids)
            )
        ...
        for review in reviews:
            self._validate_tier(review)  # 假设方法支持单条记录处理
        self._update_counter({"review_deleted": True})
